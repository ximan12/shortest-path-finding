function recursiveDivisionMaze(board, rowStart, rowEnd, colStart, colEnd, orientation, surroundingWalls) {
  ...
  if (!surroundingWalls) {
    ...
    Object.keys(board.nodes).forEach(node => {//nodes is HashMap
      if (!relevantIds.includes(node)) {//node is string r-c
  ...
  if (orientation === "horizontal") {
    ...
    let randomRowIndex = Math.floor(Math.random() * possibleRows.length);//Math.random() = [0, 1)
    ...
    //Divide and Conquer
    if (currentRow - 2 - rowStart > colEnd - colStart) {//height minus 2 is greater than width
      //top half
    ...
    if (rowEnd - (currentRow + 2) > colEnd - colStart) {
      //bottom half
      recursiveDivisionMaze(board, currentRow + 2, rowEnd, colStart, colEnd, orientation, surroundingWalls);//?should be horizontal?
    } else {
    ...
    for (let number = rowStart - 1; number <= rowEnd + 1; number += 2) {
      // we will not use the row in this orientation
      possibleRows.push(number);
    }
    let randomColIndex = Math.floor(Math.random() * possibleCols.length);
