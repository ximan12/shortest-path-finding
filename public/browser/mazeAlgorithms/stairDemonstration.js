function stairDemonstration(board) {
  //the starting point is on the bottom left
  let currentIdX = board.height - 1;
  let currentIdY = 0;
  let relevantStatuses = ["start", "target", "object"];//wall, unvisited
  while (currentIdX > 0 && currentIdY < board.width) {
    let currentId = `${currentIdX}-${currentIdY}`;
    let currentNode = board.nodes[currentId];
    let currentHTMLNode = document.getElementById(currentId);//HTMLNode = element
    if (!relevantStatuses.includes(currentNode.status)) {//current status is wall or unvisited
      currentNode.status = "wall";
      board.wallsToAnimate.push(currentHTMLNode);
    }
    //head on to upper right
    currentIdX--;
    currentIdY++;
  }
